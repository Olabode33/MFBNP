...
namespace PMSDemo.Organizations
{
    ...
    public class OrganizationUnitAppService : PMSDemoAppServiceBase, IOrganizationUnitAppService
    {
        ...
        public async Task<ListResultDto<OrganizationUnitDto>> GetOrganizationUnits()
        {
            ...
            var ouIndicatorCounts = await _unitIndicatorsRepository.GetAll()
                .GroupBy(x => x.OrganizationUnitId)
                .Select(groupedIndicators => new
                {
                    organizationUnitId = groupedIndicators.Key,
                    count = groupedIndicators.Count()
                }).ToDictionaryAsync(x => x.organizationUnitId, y => y.count);

            var ouActivityCount = await _unitActivitiesRepository.GetAll()
                .GroupBy(x => x.OrganizationUnitId)
                .Select(groupedActivities => new
                {
                    organizationUnitId = groupedActivities.Key,
                    count = groupedActivities.Count()
                }).ToDictionaryAsync(x => x.organizationUnitId, y => y.count);

            var ouDeliverableCount = await _organizationUnitRepository.GetAll().Where(x => x.ParentId != null)
                .GroupBy(x => x.ParentId)
                .Select(groupedDeliverables => new
                {
                    organizationUnitId = groupedDeliverables.Key,
                    count = groupedDeliverables.Count()
                }).ToDictionaryAsync(x => x.organizationUnitId, y => y.count);
            ...
            return new ListResultDto<OrganizationUnitDto>(
                ous.Select(ou =>
                {
                    ...
                    organizationUnitDto.IndicatorCount = ouIndicatorCounts.ContainsKey(ou.Id) ? ouIndicatorCounts[ou.Id] : 0;
                    organizationUnitDto.ActivityCount = ouActivityCount.ContainsKey(ou.Id) ? ouActivityCount[ou.Id] : 0;
                    organizationUnitDto.DeliverableCount = ouDeliverableCount.ContainsKey(ou.Id) ? ouDeliverableCount[ou.Id] : 0;
